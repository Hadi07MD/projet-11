			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				// Check If Ther's No Error Proced The Update Operation
					...
					if (empty($formErrors)) {
							...
							$now_date = $con->prepare('SELECT NOW()');
							$now_date->execute();
							$date_res = $now_date->fetchAll()[0][0];
							$stmt = $con->prepare("INSERT INTO 
													items(Name, Description, Price, Quantity, Add_Date, Country_Made, Status, Cat_ID, avatar, Member_ID, tags)
												VALUES(:zname,:zdesc, :zprice, :zquantity, :zadd_date ,:zcountry, :zstatus, :zcat, :zavatar, :zmember, :ztags)");
